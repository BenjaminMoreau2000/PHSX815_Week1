...
##
    ...
    file = open("Random Numbers.txt",'w+' )


    for i in range(0,N):
        file.write(str(myx[i])+"\n")



    #use the numbers writen to the file to recreate an array and then plot
    nums = np.genfromtxt('Random Numbers.txt',skip_header=0)[:]


    plt.hist(nums,100, density=True)
